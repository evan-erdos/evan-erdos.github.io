...
function validMove(move) {
  ...
  var tmp = move.split('-'); // moves e.g, "e2-e4", "f6-d5"
  if (tmp.length!==2) return false;
  return (validSquare(tmp[0])===true
    && validSquare(tmp[1])===true);
}
...
function validPositionObject(pos) {
  ...
  for (var i in pos) {
    ...
    if (validSquare(i) !== true
    || validPieceCode(pos[i]) !== true)
      return false;
  } return true;
}
...
function fenToObj(fen) {
  if (validFen(fen) !== true) return false;
...
function objToFen(obj) {
  if (validPositionObject(obj) !== true) return false;
  var fen = '';
  var currentRow = 8;
  for (var i = 0; i < 8; i++) {
    for (var j = 0; j < 8; j++) {
